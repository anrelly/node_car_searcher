...
export default class Parser {
    ...
    async handlePost(post) {
        ...
        let firstPageLinks = this.getLinksFromContent($, post.search.pages.page_links);
        ...
        let countPages = this.getPagenFromFirstPageContent($, post.search.pages.pagen, firstPageLinks.length);
        ...
        let pagenLinks = await this.getLinksFromPagenPages(countPages, post.url, post.search.pages.page_links, post.options.pagen);
        //console.log(pagenLinks);
    ...
    async getLinksFromPagenPages(countPages, sourceUrl, pageLinksSearchOptions, pagenOptions) {
        ...
        if (countPages > 1) {
            for (let page = 2; page <= countPages; page++) {
                ...
                //let currentUrl = sourceUrl.replace(/page=\d{1}/, `page=${page}`);
    ...
    processCurrentPageUrl(sourceUrl, pagenOptions, pageNum) {
        let rezUrl = '';
        const pagenOptionsLocal = {...pagenOptions}; // clone
    ...
    parseDetail($, detailSearchOptions, link = null) {
        const self = this;
        let fields = {...self.settings.detailFields}; // clone
        fields.link = link;

        for (const option in detailSearchOptions) {
            if (fields.hasOwnProperty(option)) {
                fields[option] = self.execCherioSelect(findEq($, detailSearchOptions[option].selector),
                    detailSearchOptions[option].method.function,
                    detailSearchOptions[option].method.args);
            }
        }
